...
- **Implicaciones para modelado**: Esta consistencia sugiere que:
  - Un modelo estacional con componentes fijas puede capturar adecuadamente el patrón
  - La variabilidad interanual afecta principalmente la amplitud, no la fase del ciclo estacional
  - Los componentes estacionales del modelo SARIMA serán robustos y predecibles
...
La estacionariedad es un requisito fundamental para muchos modelos de pronóstico de series temporales (como ARIMA). Una serie es estacionaria cuando sus propiedades estadísticas (media, varianza) no cambian con el tiempo.
...
3. **Media constante**: El promedio móvil de 12 meses (MA(12)) confirmó que la serie oscila alrededor de un nivel estable (~8-10 mm mensuales) sin cambios sistemáticos.

4. **Varianza estable**: La amplitud de las oscilaciones se mantiene relativamente constante a lo largo del período analizado, sin evidencia de heteroscedasticidad.

**Consecuencias de diferenciar innecesariamente:**

Si aplicáramos diferenciación a una serie que **ya es estacionaria**, incurriríamos en los siguientes problemas:

- **Sobrediferenciación**: Introduciríamos autocorrelación negativa artificial en lag 1
- **Pérdida de interpretabilidad**: Los coeficientes del modelo dejarían de representar relaciones directas con la precipitación
- **Mayor error de pronóstico**: La varianza del error aumentaría innecesariamente
- **Complejidad sin beneficio**: El modelo sería más difícil de ajustar y validar sin mejora en el desempeño

**Implicación para el modelado:**

Dado que la serie es estacionaria en su forma original, podemos proceder directamente con modelos de la familia ARMA o SARIMA con **d = 0** (sin diferenciación regular):

- Modelos candidatos: ARMA(p,q) o SARIMA(p,0,q)(P,D,Q)[12]
- La componente estacional bimodal puede manejarse con la parte estacional del SARIMA
- Los coeficientes del modelo se interpretarán directamente sobre los valores de precipitación en milímetros
